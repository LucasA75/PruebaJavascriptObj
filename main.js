...
//Hacer una suma de las area de los paddoks para ordenarlos decenediente

const areaOfPadocks = {}
//Recorer los paddoks para hacer la suma
paddocks.forEach(paddock => {
  //desestructuracion de objetos
    const {paddockTypeId, area} = paddock
    // areaOfPadocks[paddockTypeId] = areaOfPadocks[paddockTypeId] ?? 0
    areaOfPadocks[paddockTypeId] ??= 0  //Esto es el nullish coalesciong operator que devulve el valor de la derecha cuando el openrando de la izquierda es null
    //Aqui al areaOfPadocks le agrego el paddocktype y segun eso me suma el area cuando coincide
    areaOfPadocks[paddockTypeId] += area

})

//vamos a mapear el paddockType para que coincida con el numero de area
 const prueba = paddockType.map(p =>({
  ...p,
  totalSum:areaOfPadocks[p.id]
}))
//Los ordenamos decrecientemente
.sort((a,b)=>b.totalSum - a.totalSum)


console.log(prueba)
//Lo mapeamos para que nos muestre solo el nombre
const arrayFinal = prueba.map(p => p.name)
